...
export class PersonaController {
  ...
  async create(
    ...
    //prueba
   /*  if (p.rol == "cliente" && p.id) {
      await this.servicioAutenticacion.CrearProducto(p.id);
    }*/

    // notificar al usuario por correo
    let destino = persona.correo;
    let asunto = "Regirtro en la plataforma";
    let contenido = `Hola ${persona.nombres}, su nombre de usuario es: ${persona.correo} y su contraseña es: ${clave}`;

    fetch(`${Llaves.urlServicioNotificaciones}/envio-correo?correo_destino=${destino}&asunto=${asunto}&mensaje=${contenido}`)
      .then((data: any) => {
        console.log(data);
      })
    return p;
